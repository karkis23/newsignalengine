...
class TradingAI:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            logger.error("Error initializing model: {}".format(e))
            self.create_initial_model()
...
def predict():
    ...
    try:
        ...
        logger.info("Prediction: {}".format(result))
        
        return jsonify(result)
    
    except Exception as e:
        logger.error("Error in predict endpoint: {}".format(e))
...
def retrain():
    ...
    try:
        ...
        # Save updated model
        with open('trading_model.pkl', 'wb') as f:
            pickle.dump(trading_ai.model, f)
        with open('scaler.pkl', 'wb') as f:
            pickle.dump(trading_ai.scaler, f)
        
        return jsonify({'message': 'Model retrained successfully'})
    
    except Exception as e:
        logger.error("Error in retrain endpoint: {}".format(e))
        return jsonify({'error': str(e)}), 500
